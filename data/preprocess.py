...
def ctc_decode(predictions, max_length):
    ...
    sequence_length = tf.shape(predictions)[1]

    input_length = tf.ones(batch_size, dtype=tf.int32) * sequence_length
...
class DataHandler:
    def __init__(self, dataset: DataImporter, img_size: tuple, padding_char, start_char='', end_char=''):
        ...
        token_mask = np.zeros([len(self.char2num.get_vocabulary())], dtype=bool)
